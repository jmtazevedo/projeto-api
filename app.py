...
def login():
    ...
    # Verifica se o usuário está autenticado ou se está autenticando
    ...
    # Valida a senha no banco de dados e retorna um TOKEN
    if auth.password == usuario.senha:                           # o TOKEN é válido por 30 min
        token = jwt.encode({'id_autor': usuario.id_autor, 'exp': datetime.utcnow() + timedelta(minutes=30)}, app.config['SECRET_KEY']) 
        return jsonify({'token': token.decode('UTF-8')})
    
    # Caso a senha passada pelo usuário seja incorreta, será mostrado a tela de login novamente
    return make_response('Login inválido', 401, {'WWW-Authenticate': 'Basic realm="Login obrigatório"'})



# ============================================ #
# Rota Padrão POSTAGEM
# ============================================ #
